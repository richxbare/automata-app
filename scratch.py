...
cfgs = {
    'CFG 1': """
    S → aaA | bbA
    A → aA | bA | abaB | babB | bbbB | aaaB
    B → abaaC | abbbC | baaaC | babbC
    C → aC | bC | D
    D → aD | bE
    E →  aE | bF
    F → babG | bbaG | bbbG | abaG
    G → aG | bG | λ
    """,
    'CFG 2': """
    S → 1S | 0S | 111A | 011A | 100A | 000A
    A → 11A | 00A | 101B | 111B | 001B | 011B
    B → 1B | 0C
    C → 0C | D | E 
    D → 00D | 11F | 00F
    E → 11E | 11F | 00F
...
selected_dfa = st.selectbox("Choose a Regular Expression", ["Regular Expression 1", "Regular Expression 2"])

if selected_dfa == "Regular Expression 1":
...
if st.button("Trace DFA and Validate"):
    if user_input:
